...
public partial class Pages_Sales_SalesAccessoriesReport : System.Web.UI.Page
{
    DAL mydal = new DAL();
    ...
    protected void txtDate2_TextChanged(object sender, EventArgs e)
    {
        ...
        DataTable dt = mydal.GetdatefilterSalesAccessories(date1, date2);
        if (dt.Rows.Count > 0)
        {
    ...
    {
        DataTable dt = mydal.GetSalesAccessories();
        if (dt.Rows.Count > 0)
        {
            GridView1.DataSource = dt;
            GridView1.DataBind();
