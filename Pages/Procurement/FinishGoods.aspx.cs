...
public partial class Pages_Procurement_FinishGoods : System.Web.UI.Page
{
    DAL mydal = new DAL();
    ...
    {
        DataTable dt = mydal.GetFinishedGoodVoucher();
        txtVoucherName.Text = "CHS/FG/" + DateTime.Now.Year + "/" + dt.Rows[0][0].ToString();
    }
    ...
    protected void Gridview1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            type.DataSource = dt;
            type.DataValueField = "ProId";
            type.DataTextField = "PName";
            type.DataBind();
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        else
        {
                ...
                if (ViewState["CurrentTable"] != null)
                {
                    ...
                    if (dtCurrentTable.Rows.Count > 0)
                    {
                        for (int i = 1; i <= dtCurrentTable.Rows.Count; i++)
                        {
                            ...
                            bool FinishGoodvoucher = mydal.FinishGoodvoucher(txtVoucherName.Text, txtDate.Text, txtComment.Text, ddlItem.SelectedItem.Text,ddlModel.SelectedItem.ToString(), ddlUnit.SelectedItem.Text, txtQuantity.Text, ddlItem.SelectedValue);

                            bool Success = mydal.FinishGoods(txtVoucherName.Text, ddlItem.SelectedItem.ToString(), ddlModel.SelectedItem.ToString(), txtDate.Text, txtQuantity.Text, ddlUnit.SelectedItem.ToString());

                            DataTable searchproductinstock = mydal.searchproductinstockForProduct(ddlItem.SelectedItem.Text);

                            if (searchproductinstock.Rows.Count > 0)
                            {
                                DataTable searchPriceandqtyofproduct = mydal.Priceandqtyofproduct(ddlItem.SelectedItem.Text);
                                ...
                                bool Updateproduct = mydal.UpdateExistingProductforProduct(ddlItem.SelectedItem.Text,ddlModel.SelectedItem.ToString(), UpdatedQty, DateTime.Now);
                            }
                            else
                            {
                                bool stockbulk = mydal.InsertFinishedGoodStock(ddlItem.SelectedValue, ddlItem.SelectedItem.ToString(), txtQuantity.Text,txtDate.Text, ddlUnit.SelectedItem.Text);
                            }
                ...
                Response.Redirect("../Report/ProductionReport.aspx?id=" + txtVoucherName.Text, true);
