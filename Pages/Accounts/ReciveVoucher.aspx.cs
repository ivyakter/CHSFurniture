...
public partial class Pages_Accounts_ReciveVoucher : System.Web.UI.Page
{
    DAL mydal = new DAL();
        ...
        else
        {
            ...
            if (ViewState["CurrentTable"] != null)
            {
                ...
                if (dtCurrentTable.Rows.Count > 0)
                {
                    ...
                    {
                        DropDownList ddlclientname = (DropDownList)Gridview1.Rows[rowIndex].Cells[1].FindControl("ddlclientname");
                        TextBox txtCOA = (TextBox)Gridview1.Rows[rowIndex].Cells[2].FindControl("txtCOA");
                        TextBox txtpayment = (TextBox)Gridview1.Rows[rowIndex].Cells[4].FindControl("txtpayment");
                        TextBox txtdescription = (TextBox)Gridview1.Rows[rowIndex].Cells[4].FindControl("txtdescription");

                        string chatrofACC = "Accounts Receiveable";
                        bool InserttoPaymentrecive = mydal.insertRecivevoucher(ddlIncomeType.SelectedItem.ToString(), ddlBank.SelectedItem.ToString(), txtCheck.Text, ddlclientname.SelectedItem.Text, ddlclientname.SelectedValue, txtVoucherName.Text, txtDate.Text, txtpayment.Text, chatrofACC, ddlclientname.SelectedValue);
                        
                        if (InserttoPaymentrecive == true)
                        {
                            bool InsertIncomeVoucher = mydal.InsertIncomeVoucher(ddlclientname.SelectedValue, txtVoucherName.Text, txtdescription.Text, txtDate.Text, txtpayment.Text);
                        }
                        else
                        {
            ...
            bool InsertIncomeVoucherList = mydal.InsertIncomeVoucherList(txtVoucherName.Text, txtComment.Text, txtDate.Text, txtalltotal.Text);
    ...
    private void AddNewRowToGrid()
    {
        ...
        if (ViewState["CurrentTable"] != null)
        {
            ...
            if (dtCurrentTable.Rows.Count > 0)
            {
                for (int i = 1; i <= dtCurrentTable.Rows.Count; i++)
                {
                    DropDownList box1 = (DropDownList)Gridview1.Rows[rowIndex].Cells[1].FindControl("ddlclientname");
    ...
    protected void Gridview1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            var clientname = (DropDownList)e.Row.FindControl("ddlclientname");
            clientname.DataSource = dt;
            clientname.DataTextField = "AccName";
            clientname.DataValueField = "AccCode";
            clientname.DataBind();
            clientname.Items.Insert(0, new ListItem("--Select--", "0"));
        }
    ...
    protected void ddlclientname_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        DropDownList vendorid = (DropDownList)gr.Cells[0].FindControl("ddlclientname");
        ...
        //////// Accessories And Product Purchased amount //////

        DataTable Purchasetotal = mydal.Purchasetotal(vendorid.SelectedValue);
        ...
        DataTable Gettotalpaidamount = mydal.totalPaidamount(vendorid.SelectedValue);
        ...
        decimal Havetopay = decimal.Parse(TotalPaidAmount)-decimal.Parse(totalpurchase)  ;
