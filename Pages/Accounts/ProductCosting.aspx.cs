...
public partial class Pages_Accounts_ProductCosting : System.Web.UI.Page
{
    DAL mydal = new DAL();
    ...
    {
        if (txtDate.Text == "" && ddlproduct.SelectedItem.Text=="")
        {
            ...
            if (ViewState["CurrentTable"] != null)
            {
                ...
                if (dtCurrentTable.Rows.Count > 0)
                {
                    for (int i = 1; i <= dtCurrentTable.Rows.Count; i++)
                    {
                        ...
                        bool Success = mydal.InsertProductCosting(ddlproduct.SelectedItem.Text, ddlproduct.SelectedValue, ddlItem.SelectedItem.Text, txtQuantity.Text, txtPrice.Text, txtTotal.Text, lblcostinglast.Text, txtDate.Text);

                        if (Success==true)
                        {

                            bool UpdateToProduct = mydal.UpdateToProductPrice(ddlproduct.SelectedValue, lblcostinglast.Text);
    ...
    protected void txtQuantity_TextChanged(object sender, EventArgs e)
    {
        ...
        txtTotal.Text = ((Convert.ToDouble(txtPrice.Text == "" ? "0" : txtPrice.Text) * Convert.ToDouble(txtQuantity.Text == "" ? "0" : txtQuantity.Text)).ToString());
    ...
    protected void ddlItem_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        GridViewRow gr = item.NamingContainer as GridViewRow;
        DropDownList product = (DropDownList)gr.Cells[1].FindControl("ddlItem");
        TextBox price = (TextBox)gr.Cells[3].FindControl("txtPrice");

        DataTable dt = mydal.GetPriceFromAccessories(product.SelectedValue);
