...
public partial class Pages_Accounts_PaymentVoucher : System.Web.UI.Page
{
    DAL mydal = new DAL();
        ...
        else
        {
            ...
            if (ViewState["CurrentTable"] != null)
            {
                ...
                if (dtCurrentTable.Rows.Count > 0)
                {
                    ...
                    {
                        DropDownList ddlvendorname = (DropDownList)Gridview1.Rows[rowIndex].Cells[1].FindControl("ddlvendorname");
                        ...
                        string chatrofACC = "Accounts Payable";

                        bool InserttoPaymentvoucher = mydal.insertpaymentvoucher(ddlIncomeType.SelectedItem.ToString(), ddlBank.SelectedItem.ToString(), txtCheck.Text, ddlvendorname.SelectedItem.Text, ddlvendorname.SelectedValue, txtVoucherName.Text, txtDate.Text, txtpayment.Text, chatrofACC, ddlvendorname.SelectedValue);

                        if (InserttoPaymentvoucher == true)
                        {
                            bool InsertExpenseVoucher = mydal.InsertExpenseVoucher(ddlvendorname.SelectedValue, txtVoucherName.Text, txtdescription.Text, txtDate.Text, txtpayment.Text);                          
                        }
                        else
                        {
            ...
            bool InsertExpenseVoucherList = mydal.InsertExpenseVoucherList(ddlIncomeType.SelectedItem.ToString(),txtVoucherName.Text, txtComment.Text, txtDate.Text, txtalltotal.Text);

            Session["VoucherNo"] = txtVoucherName.Text;
            string strconfirm = "<script>if(window.confirm('Save Information Successfully!!!!')){window.location.href='../Report/ExpenseVoucherByVoucherNo.aspx'}</script>";
    ...
    {
        DataTable n = mydal.GetExpenseVoucher();
        txtVoucherName.Text = "EXP-" + n.Rows[0][0].ToString();
    }
    ...
    private void AddNewRowToGrid()
    {
        ...
        if (ViewState["CurrentTable"] != null)
        {
            ...
            if (dtCurrentTable.Rows.Count > 0)
            {
                for (int i = 1; i <= dtCurrentTable.Rows.Count; i++)
                {
                    DropDownList box1 = (DropDownList)Gridview1.Rows[rowIndex].Cells[1].FindControl("ddlvendorname");
